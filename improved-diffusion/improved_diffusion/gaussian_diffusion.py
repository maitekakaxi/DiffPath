...
class GaussianDiffusion:
    ...
    def _vb_terms_bpd_e2e(
        ...
        nll = self.token_discrete_loss_2(out["pred_xstart"], get_logits, input_ids)
        decoder_nll = decoder_nll / out["mean"].size(-1)
        decoder_nll = decoder_nll / np.log(2.0)
        #nll = nll/out["mean"].size(-1)
        #nll = nll/np.log(2.0)
        ...
        return {"output": output, "pred_xstart": out["pred_xstart"], 'kl': kl, 'decoder_nll':decoder_nll, 'kl_T':kl_T,'nll':nll}
    ...
    '''
    def token_discrete_loss_2(self, x_t, get_logits, input_ids):
        if self.model_arch == 'conv-unet' or  self.model_arch == '1d-unet':
            reshaped_x_t = x_t.view(x_t.size(0), x_t.size(1), -1).permute(0, 2, 1)
        else:
            # print(x_t.shape)
        ...
        # print(logits.shape)
        loss_fct = th.nn.CrossEntropyLoss(reduction='none')
        nll = loss_fct(logits.view(-1, logits.size(-1)), input_ids.view(-1)).view(input_ids.shape)
        # print(decoder_nll.shape)
        nll = nll.sum(dim=-1)
        return nll
    ...
    def token_discrete_loss_2(self, x_t, get_logits, input_ids):
        ...
        #valid_probs = 0.0
        for i, path in enumerate(input_ids):
            valid_probs = 0.0
            nll = 0.0
            logits_ = logits[i]  # 取出第 i 个样本的 logits
            prob = F.softmax(logits_, dim=-1)
            
            # 创建一个掩码，跳过 padding_idx 的值
            mask = (path != 2865).to(logits.device)
            
            # 只计算非 padding 的值
            valid_indices = th.arange(len(path), device=logits.device)[mask]
            valid_probs = prob[valid_indices, path[mask]]
            
            nll -= (valid_probs + 0.00001).log().sum()
            nlls.append(nll)
        nll_tensor = th.tensor(nlls, device=logits.device)
        return nll_tensor
    ...
    def calc_nll_loop(self, model, *args, **kwargs):
        if self.training_mode == 'e2e':
            return self.calc_nll_loop_e2e(model, *args, **kwargs)

    def calc_nll_loop_e2e(self, model, x_start, clip_denoised=True, model_kwargs=None, denoised_fn=None):
        device = x_start.device
        batch_size = x_start.shape[0]
