...
def denoised_fn_round(args, model, path_emb, t):
    # thresh_t = 50
    # # print(thresh_t)
    ...
    if args.model_arch == '1d-unet':
        path_emb = path_emb.permute(0, 2, 1)
    down_proj_emb = model.weight  # input_embs
    # print(t)
    old_shape = path_emb.shape
    old_device = path_emb.device

    def get_efficient_knn(down_proj_emb, path_emb, dist='l2'):
        if dist == 'l2':
            emb_norm = (down_proj_emb**2).sum(-1).view(-1, 1) #vocab
            path_emb_t = th.transpose(path_emb.view(-1, path_emb.size(-1)), 0, 1) #d, bsz*seqlen
            arr_norm = (path_emb ** 2).sum(-1).view(-1, 1) #bsz*seqlen, 1
            dist = emb_norm + arr_norm.transpose(0, 1) - 2.0 * th.mm(down_proj_emb, path_emb_t) #(vocab, d) x (d, bsz*seqlen)
            dist = th.clamp(dist, 0.0, np.inf)
        topk_out = th.topk(-dist, k=1, dim=0)

        return topk_out.values, topk_out.indices

    def get_knn(down_proj_emb, path_emb, dist='l2'):
        if dist == 'l2':
            adjacency = down_proj_emb.unsqueeze(1).expand(-1, path_emb.size(0), -1) - path_emb.unsqueeze(0).expand(
    ...
    dist = 'l2'
    if len(path_emb.shape) > 2:
        path_emb = path_emb.reshape(-1, path_emb.size(-1))
    else:
        path_emb = path_emb
    val, indices = get_efficient_knn(down_proj_emb,
                           path_emb.to(down_proj_emb.device), dist=dist)
